...
port=`expr $base_port + 11`
secure_port=`expr $base_port + 12`
...
echo "Running CORBAMemoryClient ..."

"$@" CORBAMemoryClient --secure-url="$secure_object_url" --url="$object_url"

if test $? -ne 0; then
  echo "CORBAMemoryClient return error: $?" >&2
